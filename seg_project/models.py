...
def depthwise_separable_conv(in_ch, out_ch):
...
class UNetViTDecoder(nn.Module):
    def __init__(self, num_classes=2, embed_dim=768):
        super().__init__()
        # Riduzione drastica dei canali in ingresso (Pointwise Conv)
        # Passiamo da 768 a valori molto più piccoli subito
        ...
        # Upsampling snelli
        self.up1 = PixelShuffleUpsample(256, 128)        # Out: 128
        self.up2 = PixelShuffleUpsample(128 + 128, 64)   # Out: 64 (up1 + proj3)
        self.up3 = PixelShuffleUpsample(64 + 64, 32)     # Out: 32 (up2 + proj2)
        
        # Testa finale snella ma efficace
        self.final_head = nn.Sequential(
            depthwise_separable_conv(32 + 32, 32),       # In: up3 + proj1
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
