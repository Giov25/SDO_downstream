...
def estrai_wavelenght(file_name):
    wl = file_name.split(']')[1].split('[')[1]
    return wl
def create_synchronized_filelists_samefolder(folder_path, year):
    """Create lists of files that have matching observation dates across all three data sources"""
    year_str = str(year)
    aia_list = ['131', '171', '193', '211', '304', '335', '94', '1600', '1700']
    aia_files_by_date = {wavelength: {} for wavelength in aia_list}
    M_files_by_date = {}
    ic_nl_files_by_date = {}
    for file in sorted(os.listdir(folder_path)):
        if file[1:5] in year_str:
            wl = estrai_wavelenght(file)
            if wl in aia_list:
                try:
                    Xh = read_file_header(os.path.join(folder_path, file))
                    if Xh[1]['QUALITY'] == 0:
                        date_str = file.split("[")[1].split("]")[0][:13]
                        if date_str not in aia_files_by_date[wl]:
                            aia_files_by_date[wl][date_str] = []
                        aia_files_by_date[wl][date_str].append(os.path.join(folder_path, file))
                except:
                ...
                M_files_by_date[date_str].append(os.path.join(folder_path, file))
            elif wl == 'Ic':
                date_str = file.split("[")[1].split("]")[0][:13]
                if date_str not in ic_nl_files_by_date:
                    ic_nl_files_by_date[date_str] = []
                ic_nl_files_by_date[date_str].append(os.path.join(folder_path, file))
    # Find common dates
...
def parse_args():
    ...
    parser.add_argument('--output_folder', type=str, default="/home/gpatane/Dataset/zarr_file_magnetogram_4096_definitivo.zarr")
    ...
    parser.add_argument('--do_M_list', action='store_true', default=False, help='Whether to process Magnetogram files')
...
if __name__ == "__main__":
    ...
    TARGET_RADIUS = 4096 * radius_ratio
    
    for year in years:
        print(f"Processing year {year}...")
        M_list, Ic_noLimbDark, filelist_131, filelist_171, filelist_193, filelist_211, filelist_304, filelist_335, filelist_94, filelist_1600, filelist_1700 = create_synchronized_filelists_samefolder(folder_path, year)
        ...
        if Ic_noLimbDark:
            ...
            for fn, file in tqdm(enumerate(Ic_noLimbDark), desc="Processing Ic_noLimbDark"):
                try:
                    ...
                    Xd.meta['RSUN_OBS'] = 949.80
                    Xd.meta['CDELT1']=0.504
